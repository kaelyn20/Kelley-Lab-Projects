---
title: "Stormw Water Analysis V5 - Kracken"
...
# Stormwater Analysis  
This version of the stormwater analysis file does the analysis at the family level.
...
totals <-colSums(f_counts) #get totals for each sample
rare_fcnts <- rarefy(t(fcnts[2:13]), min(totals))

richdf <- data.frame('Sample' = as.factor(names(rare_fcnts)), 'Richness' = rare_fcnts)
...
for (i in 1:12) { 
  richness[i, 2] <- length(which(f_counts[i]>0)) #counts number of times there's a counts greater than 0
  richness[i, 1] <- colnames(f_counts[i]) 
}
...
colnames(r_abund) <- colnames(fcnts) #rename the columns as the same as gcnts
r_abund[1] <- fcnts[1]

for(i in 2:13) {
  r_abund[i] <- round(fcnts[i]/totals[i-1], 4) #divide count of phylum of given site by the total number of features for that site
}
...
ggplot(tidy_above1, aes(x=Sites, y=Abundances, fill=family)) +
  geom_bar(stat="identity") + theme_classic()

ggplot(tidy_above2, aes(x=Sites, y=Abundances, fill=family)) +
  geom_bar(stat="identity") + 
  theme_classic()
...
f_counts[f_counts == 0] <- .01
clr <- function(x) sweep(log(x), 1, rowMeans(log(x)), "-")
bac_tx <- t(clr(t(f_counts))) #clr transform data
